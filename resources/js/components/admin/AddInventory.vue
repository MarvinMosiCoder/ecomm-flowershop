                        ...
                        <thead class="text text-success">
                            ...
                            <tbody>
                                <tr v-for="(tableData,index) in tableRows" :key="index">                            
                                    ...
                                    <td>
                                        <!-- <select2 placeholder="Choose Flower Type" v-model="tableData.selectedData" :options="finalLineOptions[index]" name="test_option[]" :id="'test_option'+index" @change="optionChange" :settings="{width:'100%'}"/> -->
                                        <select :id="'test_option'+index" @change="optionChange">
                                            <!-- <option v-for="datas in finalLineOptions[index]" :key="datas.id" :value="datas.id" >{{ datas.text }}</option> -->
                                        </select>
                                    </td> 
                                    <td>
...
const {isNumber} = jsHelper();
    ...
    export default {
        ...
        methods: {
            ...
            optionChange(e){
                this.lineOptionStack.push(parseInt(e.srcElement.value));
            ...
            select(itemId){
                ...
                .then((response)=>{
                    ...
                    for (x = 0; x < this.lineOptions.length; ++x) {               
                        var j = x + 1;
                        showData[j] = "<option value='"+ this.lineOptions[x].id+"'>" + this.lineOptions[x].text +"</option>";
                    }
                    //this.tableRows[this.index].selectedData = showData;
                    $('#test_option'+this.index).html(showData);   
                    this.index++;
                })
            },
